...
<div class="rounded-2xl bg-[#19152E] p-[30px] w-max">
    ...
    <form id="delete-subscription" action="{{ route('member.user_premium.destroy',$user_premium->id) }}" method="POST">
        @csrf
        @method('DELETE')
        <button id="delete-subscription-btn" type="submit" class="px-[19px] py-[13px] bg-[#FE4848] rounded-full text-center">
            <span class="text-white font-semibold text-base">
                Stop Subscribe
            </span>
        </button>
...
@section('js')
    <script>
        document.getElementById('delete-subscription-btn').addEventListener('click',function(event){
            event.preventDefault();
            const confirmDelete=confirm('Apakah kamu yakin ingin berhenti berlangganan?');
            if(confirmDelete){
